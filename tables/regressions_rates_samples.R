...
c3_search_rate <- stops_panel %>% 
  filter(year < 2020) %>% 
  feols(search_rate ~treatment | district + date,
        cluster = ~district)


c4_search_rate <- stops_panel %>% 
  filter(year < 2020) %>% 
  filter(never_treated == 0) %>% 
  feols(search_rate ~treatment | district + date,
        cluster = ~district)
...
c3_pat_rate <- stops_panel %>% 
  filter(year < 2020) %>% 
  feols(patdown_rate ~treatment | district + date,
        cluster = ~district)


c4_pat_rate <- stops_panel %>% 
  filter(year < 2020) %>% 
  filter(never_treated == 0) %>% 
  feols(patdown_rate ~treatment | district + date,
        cluster = ~district)
...
c3_firearm_rate <- stops_panel %>% 
  filter(year < 2020) %>% 
  feols(firearm_found_rate ~treatment | district + date,
        cluster = ~district)


c4_firearm_rate <- stops_panel %>% 
  filter(year < 2020) %>% 
  filter(never_treated == 0) %>% 
  feols(firearm_found_rate ~treatment | district + date,
        cluster = ~district)
...
rates <- panelsummary(list(c1_search_rate, c2_search_rate,
                           c3_search_rate, c4_search_rate),
                      list(c1_pat_rate, c2_pat_rate,
                           c3_pat_rate, c4_pat_rate),
                      list(c1_firearm_rate, c2_firearm_rate,
                           c3_firearm_rate, c4_firearm_rate),
                      mean_dependent = T,
                      collapse_fe =T,
  ...
  add_header_above(c(" " = 1, 
                     ...
                     "All" = 1,
                     "Only Treated" = 1)) %>% 
  add_header_above(c(" " = 1, "2016-2022"= 2, "2016-2019" = 2)) %>% 
