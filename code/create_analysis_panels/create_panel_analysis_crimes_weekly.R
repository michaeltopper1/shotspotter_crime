...
## starts on sundays
panel_dates <- seq(as_date("2015-12-27"), as_date("2023-01-02") , by= "week") %>% 
  as_tibble() %>% 
  rename(week = value) 
...
crimes_panel <- panel_dates %>% 
  left_join(crimes_aggregated, by = join_by(week == week, 
                                            district == district))
...
crimes_panel <- crimes_panel %>% 
  ...
  mutate(treatment = ifelse(shotspot_activate <= week, 1, 0), .by = district) %>% 
...
crimes_panel %>% 
  write_csv("analysis_data/crimes_panel_weekly.csv")
