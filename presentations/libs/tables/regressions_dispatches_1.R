## Purpose of script:
##
## Author: Michael Topper
##
## Date Last Edited: 2023-05-14
##

library(tidyverse)
library(fixest)
library(panelsummary)
library(kableExtra)
library(did2s)



if (!exists("dispatch_panel")){
  dispatch_panel <- read_csv(here::here("analysis_data/xxdispatch_panel.csv"))
  
}


# panel A -----------------------------------------------------------------

## Entry to dispatch
entry_1 <- feols(entry_to_dispatch_1 ~ treatment + officer_hours +
                   number_dispatches_1 + number_dispatches_2 +
                   number_dispatches_3 + number_dispatches_0| district + date,
                 cluster = ~district,
                 data = dispatch_panel)

entry_2 <- did2s(data = dispatch_panel,
                 yname = "entry_to_dispatch_1",
                 first_stage = ~officer_hours +
                   number_dispatches_1 + number_dispatches_2 +
                   number_dispatches_3 + number_dispatches_0| district + date,
                 second_stage = ~treatment,
                 treatment = "treatment",
                 cluster_var = "district")

# entry_3 <- dispatch_panel %>% 
#   feols(entry_to_dispatch_1 ~ treatment + shotspot_border_treatment + officer_hours +
#           number_dispatches_1 + number_dispatches_2 +
#           number_dispatches_3 + number_dispatches_0| district + date,
#         cluster = ~district)


entry_4 <- dispatch_panel %>% 
  feols(entry_to_dispatch_2~ treatment  +officer_hours +
          number_dispatches_1 + number_dispatches_2 +
          number_dispatches_3 + number_dispatches_0| district + date)

entry_5 <- dispatch_panel %>% 
  feols(entry_to_dispatch_3~ treatment +officer_hours +
          number_dispatches_1 + number_dispatches_2 +
          number_dispatches_3 + number_dispatches_0| district + date,
        cluster = ~district)





# entry to onscene --------------------------------------------------------


entry_os_1 <- dispatch_panel %>% 
  feols(entry_to_onscene_1~ treatment  + officer_hours +
          number_dispatches_1 + number_dispatches_2 +
          number_dispatches_3 + number_dispatches_0| district + date,
        cluster = ~district)

entry_os_2 <- did2s(data = dispatch_panel,
                    yname = "entry_to_onscene_1",
                    first_stage = ~officer_hours +
                      number_dispatches_1 + number_dispatches_2 +
                      number_dispatches_3 + number_dispatches_0| district + date,
                    second_stage = ~treatment,
                    treatment = "treatment",
                    cluster_var = "district")

# entry_os_3 <- dispatch_panel %>% 
#   feols(entry_to_onscene_1~ treatment + shotspot_border_treatment + officer_hours +
#           number_dispatches_1 + number_dispatches_2 +
#           number_dispatches_3 + number_dispatches_0| district + date,
#         cluster = ~district)


entry_os_4 <- dispatch_panel %>% 
  feols(entry_to_onscene_2 ~ treatment  + officer_hours +
          number_dispatches_1 + number_dispatches_2 +
          number_dispatches_3 + number_dispatches_0| district + date)

entry_os_5 <- dispatch_panel %>% 
  feols(entry_to_onscene_3~ treatment  + officer_hours +
          number_dispatches_1 + number_dispatches_2 +
          number_dispatches_3 + number_dispatches_0| district + date,
        cluster = ~district)


# -------------------------------------------------------------------------


gof_mapping <- tribble(~raw, ~clean, ~fmt,
                       "nobs", "Observations", 0,
                       "FE: date", "FE: Day-by-Month-by-Year", 3,
                       "FE: district", "FE: District", 3)

footnotes <- map(list("* p < 0.1, ** p < 0.05, *** p < 0.01"), ~str_remove_all(., "\n"))



dispatch_table_1 <- panelsummary_raw(list(entry_1, entry_2,
                                          entry_4, entry_5),
                                     stars = "econ",
                                     mean_dependent = T,
                                     coef_map = c( "treatment" = "ShotSpotter Activated",
                                                   "shotspot_border_treatment" = "Border Activated"),
                                     gof_omit = "^R|A|B|S",
                                     gof_map = gof_mapping) %>% 
  mutate(`Model 2` = if_else(term == "FE: District" |
                               term == "FE: Day-by-Month-by-Year", "X", `Model 2`)) %>% 
  add_row(term = "Gardner (2021) Robust Estimator",
          `Model 1` = "",
          `Model 2` = "X",
          `Model 3` = "",
          `Model 4` = "") %>% 
  mutate(`Model 2` = if_else(term == "Mean of Dependent Variable", 
                               `Model 1`, `Model 2`)) %>% 
  mutate(across(starts_with("M"), ~if_else(term == "Observations",
                                           . %>% as.double() %>% as.integer() %>% scales::comma(), .))) %>% 
  clean_raw() %>% 
  add_header_above(c(" " = 1,
                     "Priority 1" = 2,
                     "Priority 2" = 1,
                     "Priority 3" = 1)) %>% 
  footnote(footnotes, threeparttable = T) %>% 
  kable_classic(full_width = F, html_font = "Cambria")


dispatch_table_2 <- panelsummary_raw(list(entry_os_1, entry_os_2,
                                          entry_os_4, entry_os_5),
                                     stars = "econ",
                                     mean_dependent = T,
                                     coef_map = c( "treatment" = "ShotSpotter Activated",
                                                   "shotspot_border_treatment" = "Border Activated"),
                                     gof_omit = "^R|A|B|S",
                                     gof_map = gof_mapping) %>% 
  mutate(`Model 2` = if_else(term == "FE: District" |
                               term == "FE: Day-by-Month-by-Year", "X", `Model 2`)) %>% 
  add_row(term = "Gardner (2021) Robust Estimator",
          `Model 1` = "",
          `Model 2` = "X",
          `Model 3` = "",
          `Model 4` = "") %>% 
  mutate(`Model 2` = if_else(term == "Mean of Dependent Variable", 
                             `Model 1`, `Model 2`)) %>% 
  mutate(across(starts_with("M"), ~if_else(term == "Observations",
                                           . %>% as.double() %>% as.integer() %>%  scales::comma(), .))) %>% 
  clean_raw() %>% 
  add_header_above(c(" " = 1,
                     "Priority 1" = 2,
                     "Priority 2" = 1,
                     "Priority 3" = 1)) %>% 
  footnote(footnotes, threeparttable = T) %>% 
  kable_classic(full_width = F, html_font = "Cambria")

