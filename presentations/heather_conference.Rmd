...
## State of Crime in the US:
  - Gun violence has increased relative to pre-Covid
  - Calls to defund the police
  - More mistrust of the police
  - Higher police officer churn]
...
.font110[
- Rapid response  most important (Collge of Policing 2013)
...
## Empirical Strategy:
  - Staggered difference-in-difference
    - Daily level
  - Variation: ShotSpotter rollouts

## Main Results:
  - Longer dispatch/arrival times:
    - +2.5 minutes in call-to-dispatch
    - +3 minutes in call-to-on-scene
  - Lower arrest rates
    - 5% decrease in arrests made
]
---
# Contribution:
...
- Non-Econonomics: 
  - No violent crime effect, mixed response time results (Mares and Blackburn 2021)
  - Lower hospital travel time (Goldenberg et al. 2019)
  - Lowered response times (Mazerolle et al. 1998)
]
...
<font color="blue"> We find evidence of lower gun victomization, linked directly to a police operating procedure. </font>
- Alternative Studies:
...
### High Priority
  - Priority 1 (immediate dispatch)
      - Treated similarly to an active shooter
...
```{r, eval = T}
source("libs/figures/distribution_outcomes.R")
```

```{r distribution, out.width = "850px", out.height = "550px", fig.align="center", eval = T}
...
* $Y_{d,t}$ is an outcome of average <b>daily</b> time (in seconds) in police district $d$ in time $t$ 
  ...
  - 911 Call-to-On-Scene 
* $ShotSpotter_{d,t}$ is an indicator variable when ShotSpotter technology is activated in police district $d$ in time $t$
...
* $\mathbb{X}_{d,t}$ is a vector of controls:
    - Officer Hours
    - Number of 911 Calls (by priority)
...
.panelset[
.panel[.panel-name[Effect on Call-to-Dispatch]
```{r}
dispatch_table_1
```
]
...
.panel[.panel-name[Effect on Call-to-On-Scene]
```{r}
dispatch_table_2
```
]

.panel[.panel-name[Effect on Arrest Rate]
```{r}
arrest_rate_table
```
]
...
---
# Call to On-Scene Times (seconds): Top 5
```{r}
forestplot_priority_call_onscene
```
...
- First causal policy analysis of ShotSpotter:
    - Unintended Consequences:
      - Higher Dispatch/On-Scene Times for Priority 1 (2.5/3.0 minutes)
      - Lower Arrest Rates for Priority 1 (5% decrease)
    - Intended Consequence 
      - Lower Gun Victimization
