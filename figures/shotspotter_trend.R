...
time_graph <- dispatch_panel %>% 
  ...
  mutate(type = if_else(type =="number_shots", "ShotSpotter Dispatches", 
                        "Civilian Reports of Gunshots")) %>% 
  filter(type == "ShotSpotter Dispatches") %>% 
...
ggsave(shotspotter_trend, filename = "presentations/figures/shotspotter_trend.jpeg",
       width = 7, height = 5)
