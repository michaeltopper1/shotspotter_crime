...
aggregate_calls <- dispatch_panel_with_shots %>% 
  group_by(date, district) %>% 
  summarize(number_dispatches = n()) %>% ungroup()


aggregate_p1_calls <- dispatch_panel_with_shots_1 %>% 
  group_by(date, district) %>% 
  summarize(number_dispatches_p1 = n()) %>% ungroup()

aggregate_calls_p2 <- dispatch_panel_with_shots %>% 
  filter(priority_code ==2) %>% 
  group_by(date, district) %>%
  summarize(number_dispatches_p2 = n()) %>% ungroup()


aggregate_calls_p3 <- dispatch_panel_with_shots %>% 
